...
-- re.util.include_cl("bodyman/bodyman_config.lua")
...
function BODYMAN:Chat( msg )
	chat.AddText(HexColor("#078dff"), "gm.hub / Bodygroups | ", Color(255,255,255), msg)
end
...
function BODYMAN:OpenMenu()
	...
	frame:Center()
	frame:SetPrimaryColor( HexColor("#078dff") )
	frame:SetSecondaryColor( HexColor("#078dff", 4) )
	frame:SetTitle("")
	frame:MakePopup()
	...
	function pmodel:PaintOver( w, h )
		...
		ArizardShadowText(BODYMAN.HelpText, luna.MontBase24, 0,0, Color(255,255,255), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP, 1 )
	end
...
function BODYMAN:RefreshAppearanceMenu( )
	...
	if type( playermodels ) == "table" then
		...
		for k=1,#playermodels do
			...
			btn:SetColors( Color(0, 0, 0, 150), Color(0, 0, 0, 150))
	...
	if allowedbodygroups != {} then
		...
		bgtitle:SetFont(luna.MontBase24)
		...
		for bgname, bgtable in pairs( allowedbodygroups ) do
			...
			bglabel:SetFont(luna.MontBase18)
...
function BODYMAN:AdminMenu()
	...
	frame:SetPrimaryColor( HexColor("#078dff") )
	frame:SetSecondaryColor( HexColor("#078dff", 4) )
	frame:SetTitle("Grupy ciała Admin Menu")
	frame:MakePopup()
