...
function OpenSpawnEditorMenu()
	...
	sheet:AddSheet( "Добавление точек", CreatePanel, "icon16/add.png" )

	local EditPanel = vgui.Create( "DPanel", sheet )
	EditPanel:DockMargin(5, 10, 5, 5)
	EditPanel.Paint = function( self, w, h ) end 
	sheet:AddSheet( "Управление спавнами", EditPanel, "icon16/application_edit.png" )

	local AdminPanel = vgui.Create( "DPanel", sheet )
	AdminPanel:Hide()
	if LocalPlayer():IsSpawnPointSuperAdmin() then
		AdminPanel:Show()
		sheet:AddSheet( "Панель разработчика", AdminPanel, "icon16/wrench.png" )
	end
	...
	PrioSlider:SetText( "Приоритет точки спавна" )
	PrioSlider:SetMin( 0 )	
	...
	JobList:AddColumn( "Категория" )
	JobList:AddColumn( "Профессия" )
	...
	DLabel:SetText( "Чтобы выбрать несколько профессий - удерживайте CTRL" )
	...
	BeginSpawnCreation:SetText( "Начать" )
	...
	DeleteMapSpawns:SetText( "Удалить спавны для текущей карты" )
	DeleteMapSpawns:SetIcon("icon16/map_delete.png")
	DeleteMapSpawns.DoClick = function()
		Derma_Query(
		    "Вы действительно хотите удалить все точки спавна для этой карты?",
		    "Удаление точек спавна текущей карты",
		    "Удалить навсегда",
		    function() 
	...
	DeleteAllSpawns:SetText( "Очистить базу данных" )
	DeleteAllSpawns:SetIcon("icon16/database_delete.png")
	DeleteAllSpawns.DoClick = function()
		Derma_Query(
		    "Вы действительно хотите удалить ВСЕ точки спавна? Может лучше удалить только для текущей карты?",
		    "Удаление точек спавна из базы данных",
		    "Удалить навсегда",
		    function()
		    	DeleteMapSpawns:SetEnabled(false)
	...
	SpawnList:AddColumn( "Приоритет" )  :SetWidth(SpawnList:GetWide()*0.25)
	SpawnList:AddColumn( "Расстояние" ) :SetWidth(SpawnList:GetWide()*0.3)
	SpawnList:AddColumn( "Профессий" )  :SetWidth(SpawnList:GetWide()*0.3)
	...
	AllowedJobs:AddColumn( "Профессия" )
	...
	Button_Add_Job:SetText( "Добавить профессию" )
	...
	Button_Remove_Job:SetText( "Убрать профессию" )
	...
	Button_Save_Jobs:SetText( "Сохранить" )
	...
	DLabel:Dock( TOP )
	DLabel:DockMargin(5, 0, 0, 0)
	DLabel:SetText( "Приоритет: (от 0 до 100)" )
	...
	DLabel:SetWrap( true )
	DLabel:SetAutoStretchVertical(true)
	DLabel:SetText( "Игроки всегда появляются на случайной точке с максимальным приоритетом." )
	...
	Button_Goto_Spawn:SetText( "Телепортироваться" )
	...
	Button_Delete_Spawn:SetText( "Удалить точку спавна" )
	Button_Delete_Spawn.DoClick = function()
		Derma_Query(
		    "Вы действительно хотите безвозвратно удалить точку спавна?",
		    "Удаление точки спавна "..selected_spawn_id,
		    "Удалить навсегда",
		    function() 
		    	Button_Delete_Spawn:SetEnabled(false)
