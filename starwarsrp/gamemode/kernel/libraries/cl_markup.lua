...
local function ExtractParams(p1, p2, p3)
	if string.utf8sub(p1, 1, 1) == "/" then
		...
		elseif p1 == "img" and p2 then
			...
			local p3 = exploded[2]
			local r = exploded[3] or 255
			local g = exploded[4] or 255
			local b = exploded[5] or 255
			local a = exploded[6] or 255
...
function MarkupObject:draw(xOffset, yOffset, halign, valign, alphaoverride)
	for i = 1, #self.blocks do
		...
		if blk.texture then
			...
			surface.SetDrawColor(blk.color.r, blk.color.g, blk.color.b, alphaoverride or blk.color.a or 255)
