...
function ENT:ClimbTrace()
	local tracedata = {
		...
		endpos = self:LocalToWorld( self:OBBCenter() + Vector(150,0,0) ),
		filter = function( ent ) 
...
function ENT:GetMoveXY( ent, phys, deltatime )
	...
	if (moveSpeed >= 10) then
		...
		-- self:SetPlaybackRate(moveSpeed/333)
		self:PlayAnimation("walk", moveSpeed/175)
		-- self:ResetSequence(self:LookupSequence( "walking" ))
		-- self:SetPlaybackRate(moveSpeed/75)
