...
ENT.ExplosionEffect = "lvs_explosion_white"
...
if SERVER then
    ...
    function ENT:GetDamage() return (self._dmg or 500) end
    function ENT:GetRadius() return (self._radius or 500) end
    ...
    function ENT:Initialize()    
        ...
        -- Start a timer to spawn cluster missiles after x seconds
        timer.Simple(2, function()
            if IsValid(self) then
                self:SpawnClusterMissiles()
				-- self:Remove()
				 self:Detonate()
            end
        end)
    ...
    function ENT:Think()
        ...
        -- Commented out to avoid detonation after 12 seconds
    ...
    function ENT:SpawnClusterMissiles()	
		--self:EmitSound("lvs/vehicles/clone_attack_drone/shot_3.wav", 200 )
		local numMissiles = 30
		local clusterRadius = 400  -- Adjust this radius as needed

		-- Get the original missile's forward direction
		local originalDirection = self:GetForward()
		local originalAngles = self:GetAngles()

		for i = 1, numMissiles do
			local angle = 360 * (i - 1) / numMissiles
			local offset = Vector(math.cos(math.rad(angle)), math.sin(math.rad(angle)), 0) * clusterRadius
			local spawnPos = self:GetPos() + offset
			timer.Simple(i * 0.05, function()
				local projectile = ents.Create("lvs_firemissile")
				if IsValid(projectile) then
					local newAngles = originalAngles  -- Use the original missile's angles
					local newDirection = newAngles:Forward()

					projectile:SetPos(spawnPos)
					projectile:SetAngles(newAngles)
					...
					projectile:Spawn()
					projectile:Activate()
					projectile:SetSpeed(originalSpeed)
    ...
    function ENT:Enable()
        ...
        self.snd:SetSoundLevel(20)
        self.snd:Play()

        local effectdata = EffectData()
            effectdata:SetOrigin(self:GetPos())
            effectdata:SetEntity(self)
        util.Effect("lvs_missiletrail_white", effectdata)
    end
