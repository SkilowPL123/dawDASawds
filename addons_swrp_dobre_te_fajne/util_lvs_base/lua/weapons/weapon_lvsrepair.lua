...
if CLIENT then
	SWEP.PrintName		= "Фузионный Резак"
	SWEP.Author			= "Blu-x92"
	...
	function SWEP:DrawHUD()
		...
		if not IsValid( ply ) or not ply:KeyDown( IN_ATTACK2 ) then
			...
			if IsValid( lvsEnt ) and (Pos - ply:GetShootPos()):Length() < self.MaxRange and not ply:InVehicle() then
				DrawText( ply:GetEyeTrace().HitPos, "Техника\nЗдоровье: "..math.Round(lvsEnt:GetHP()).."/"..lvsEnt:GetMaxHP(), ColorText )
			end
...
function SWEP:PrimaryAttack()
	...
	if IsValid( prop ) and prop.isFort then
		prop:EmitSound("physics/concrete/concrete_break2.wav")
		prop:Remove()
		return
	end
	...
	Target:SetHP( math.min( HP + 150, MaxHP ) )
