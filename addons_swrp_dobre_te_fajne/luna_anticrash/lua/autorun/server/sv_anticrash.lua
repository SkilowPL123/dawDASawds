...
function HPP.DeLag()
    if aprint then
	   aprint(Color(255, 0, 0), "Meta.Protection | ", {"moderator", "founder"}, "Staramy się wyeliminować opóźnienia.")
    end
...
function HPP.StopLag()
    if aprint then
        aprint(Color(255, 0, 0), "Meta.Protection | ", nil, "The server was lagging so all entities have been frozen.")
        aprint(Color(255, 0, 0), "Meta.Protection | ", {"root", "superadmin", "headadmin", "admin", "mod"}, "If this keeps occuring someone may be trying to crash the server.")
    end
	for k, v in pairs(ents.GetAll()) do
...
-- print(engine.RealFrameTime(), "fps")
