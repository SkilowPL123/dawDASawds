...
SWEP.Category			= "[LVS]"
SWEP.PrintName		= "Wyrzutnia rakiet PLX-1 (wersja 2)"
SWEP.Author			= "Skip"
SWEP.Slot				= 4
SWEP.SlotPos			= 9
...
function SWEP:Think()
	...
	if self.FindTime + 2 < curtime and IsValid( self:GetClosestEnt() ) then
		self.Locked = true
	else
		self.Locked = false
	end
	...
	if self.guided_nextThink < curtime then
		...
		for k, v in pairs( self.FoundVehicles  ) do
			if IsValid( v ) then
				...
				if Ang < 5 and dist < 15000 and self:CanSee( v ) then -- Tracking angle necessary to track and max track distance 
...
function SWEP:PrimaryAttack()
	...
	local ent = ents.Create( "stinger_missile" )
	ent:SetPos( startpos + Vector(0,0,0) + self.Owner:EyeAngles():Forward() * 150)
