...
function PlayerSpawnBodygroups( ply )
	...
	local job = re.jobs[ply:Team()] -- this is the part where it checks for job bodygroups, and applies them if need be
	-- local skins = {}
	local bodygroups = {}

	timer.Simple(1, function()
		if job and ply.LastJob_bodygroupr == job.command then
	...
	if job and job.bodygroups then
