...
SWEP.VElements = {
	["bucket"] = { type = "Model", model = "models/johnsheppard44/caliber/military/equipment/armor refill bag.mdl", bone = "ValveBiped.cube", rel = "", pos = Vector(0,1, -1.593), angle = Angle(-69.873, 35, -15), size = Vector(0.25, 0.25, 0.25), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}


SWEP.WElements = {
	["bucket"] = { type = "Model", model = "models/johnsheppard44/caliber/military/equipment/armor refill bag.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(3.17, 5.129, 0.8), angle = Angle(5.981, 1, 180), size = Vector(0.5, 0.5, 0.5), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
function SWEP:PrimaryAttack()
    ...
    if self.Atypes == 1 then
	   if SERVER then
		  local grenade = ents.Create("healthbox")
    ...
    if self.Atypes == 2 then
	   if SERVER then
		  local grenade = ents.Create("armorbox")
		  --grenade:SetOwner(self.Owner)
		  grenade:SetPos(self.Owner:EyePos() + (self.Owner:GetAimVector() * 50))
		  grenade:SetAngles(self.Owner:GetAngles())
            self.Owner:MuzzleFlash()
    ...
    -- Third cycle (RPG)
	   ...
	   if SERVER then
		  local grenade = ents.Create("engbox")
		  --grenade:SetOwner(self.Owner)
		  grenade:SetPos(self.Owner:EyePos() + (self.Owner:GetAimVector() * 50))
		  grenade:SetAngles(self.Owner:GetAngles())
            self.Owner:MuzzleFlash()
