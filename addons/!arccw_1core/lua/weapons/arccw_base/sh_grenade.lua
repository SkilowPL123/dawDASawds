...
if !inf and (bot and self:Ammo1() or self:Clip1()) < aps then
    if self:Ammo1() == 0 and self:Clip1() == 0 and !self:GetBuff_Override("Override_KeepIfEmpty", self.KeepIfEmpty) then
        ...
        -- Ensure owner is valid and has StripWeapon method
        if IsValid(owner) and owner:IsPlayer() and owner.StripWeapon then
    ...
    local owner = self:GetOwner()
    local alt = IsValid(owner) and owner:IsPlayer() and owner:KeyDown(IN_ATTACK2)    self:SetGrenadeAlt(alt)
