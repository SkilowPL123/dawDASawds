...
lunaLegs.legEnt = lunaLegs.legEnt || nil
lunaLegs.playBackRate = 1
lunaLegs.sequence = nil
lunaLegs.velocity = 0
lunaLegs.oldWeapon = nil
lunaLegs.breathScale = 0.5
lunaLegs.nextBreath = 0
lunaLegs.renderAngle = nil
lunaLegs.biaisAngle = nil
lunaLegs.radAngle = nil
lunaLegs.renderPos = nil
lunaLegs.renderColor = { }
lunaLegs.clipVector = vector_up * -1
lunaLegs.forwardOffset = -20
lunaLegs.nextMatSet = CurTime( )
lunaLegs.lastRoleName = lunaLegs.lastRoleName || 0
lunaLegs.lastCharName = lunaLegs.lastCharName || 0
...
function META:ShouldDrawLegs()
  ...
  return lunaLegs.legEnt && lunaLegs.legEnt:IsValid() && self:Alive()
  && !self:InVehicle() && self:GetViewEntity() == self
  && !self:ShouldDrawLocalPlayer() && !self:GetNoDraw() && self:GetObserverTarget() == NULL && EyeAngles().p > 0

end

function lunaLegs:CreateLegs()
  ...
  self.lastRoleName = ply:Team() || "none"
...
function lunaLegs:PlayerWeaponChanged( ply, weapon )
...
--function lunaLegs:UpdateAnimation( ply, velocity, speed )
...
--end
--hook.Add( "UpdateAnimation", "Legs_UpdateAnimation", lunaLegs.UpdateAnimation )

function lunaLegs:LegsWork( ply, speed )
  ...
  if ( self.lastCharName != ply:GetName() or self.lastRoleName != ply:Team() ) then
...
function lunaLegs:RenderScreenspaceEffects()

  local ply = LocalPlayer()
  local plytable = ply:GetTable()
  --if ( !ply:IsSolid() || ( ply:GTeam() == team_index_scp && !plytable.IsZombie && !allowedscp[ply:Team()] ) ) then return end
...
hook.Add( "PostDrawOpaqueRenderables", "Legs_ScreenSpaceEffects", lunaLegs.RenderScreenspaceEffects )


hook.Add("UpdateAnimation", "afsafas", function(player, velocity, maxSeqGroundSpeed)
    if ( player == LocalPlayer() ) then

        if ( lunaLegs.legEnt && lunaLegs.legEnt:IsValid() ) then
  
          lunaLegs:LegsWork( player, maxSeqGroundSpeed )
