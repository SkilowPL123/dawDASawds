...
if SERVER then
	...
	function ENT:GetSpeed() return (self._speed or 25) end
...
function ENT:Detonate(target)
    ...
    -- Spawn fire entity with correct owner
    local fire = ents.Create("lvs_fire")
    if IsValid(fire) then
        fire:SetPos(self:GetPos())
        fire:SetOwner(game.GetWorld())
        fire:Spawn()
    end
