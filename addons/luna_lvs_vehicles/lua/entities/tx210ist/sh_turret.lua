...
function ENT:InitTurret()
	...
	weapon.Attack = function( ent )
		...
		self:EmitSound( "lvs/slavei/atatw.wav" )
		...
		if IsValid( PhysObj ) then
			PhysObj:ApplyForceOffset( Muzzle.Ang:Right() * 100000, Muzzle.Pos )
		end
	...
	weapon.CalcView = function( ent, ply, pos, angles, fov, pod )
		...
		local StartPos = self:LocalToWorld( Vector(0,0,100) )
	...
	weapon.Attack = function( ent )
		...
		local PhysObj = base:GetPhysicsObject()
		if IsValid( PhysObj ) then
			PhysObj:ApplyForceOffset( muzzle.Ang:Right() * 1000, muzzle.Pos )
		end
	end
	weapon.OnThink = function( ent, active )
		local base = ent:GetVehicle()
