...
function ENT:Draw()
	...
	if self:GetPos():Distance(LocalPlayer():GetPos()) < 300 then
		...
		cam.Start3D2D(self:GetPos()+self:GetUp()*35, Ang, 0.05)
			render.PushFilterMin(TEXFILTER.ANISOTROPIC)
				...
				--draw.Icon(icon_size*-.6,icon_size*-.5-90,icon_size,icon_size,mat_wep1,color_white)
				draw.ShadowSimpleText( 'Płyty pancerne', "font_npc1", -3, 0, COLOR_SECONDARY, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, Color(0, 0, 0, 255))
				--draw.ShadowSimpleText( 'Бронепластины', "font_npc1neon", -3, 0, COLOR_SECONDARY, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, Color(0, 0, 0, 255))
				draw.ShadowSimpleText( 'Daje standardowe 200 punktów pancerza.', "font_npc2", -3, 70, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, Color(0, 0, 0, 255))
