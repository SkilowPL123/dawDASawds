...
hook.Add( "HUDPaint", "40kFieldGunDrawHUD", function()
    ...
    if IsValid(seat) and IsValid(seat:GetParent()) and (seat:GetParent():GetClass() == "sent_40k_fieldcannon") then
        ...
        -- Найдите индекс крепления
        local attachmentIndex = Fieldcannon:LookupAttachment(Fieldcannon:GetNWString("AimAttachment", 0))
        
        -- Проверьте, существует ли крепление
        if attachmentIndex > 0 then
            local AimAttachment = Fieldcannon:GetAttachment(attachmentIndex)

            -- Если крепление найдено
            if AimAttachment then
                ...
                print("Błąd: nie znaleziono mocowania AimAttachment!")
            end
        else
            print("Błąd: nie udało się znaleźć indeksu mocowania AimAttachment!")
        end
    end
end)
