...
SWEP.Category			= "[LVS]"
SWEP.PrintName		= "Wyrzutnia rakiet PLX-1 (wersja 1)"
SWEP.Author			= "Skip"
SWEP.Slot				= 4
SWEP.SlotPos			= 9
...
function SWEP:Think()
	...
	if self.FindTime + 3.5 < curtime and IsValid( self:GetClosestEnt() ) then
		self.Locked = true
	else
		self.Locked = false
	end
	...
	if self.guided_nextThink < curtime then
		...
		for k, v in pairs( self.FoundVehicles  ) do
			if IsValid( v ) then
				...
				if Ang < 10 and dist < 10000 and dist > 3000 and self:CanSee( v ) then -- Tracking angle necessary to track and max track distance 
...
function SWEP:PrimaryAttack()
	...
	local ent = ents.Create( "javelin_missile" )
	ent:SetPos( startpos + self.Owner:EyeAngles():Forward() * 15)
	ent:SetAngles(Angle(270,0,0))
