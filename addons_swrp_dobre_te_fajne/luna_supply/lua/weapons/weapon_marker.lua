...
SWEP.Supply_Coldown             = 100
SWEP.Supply_icon = Material("luna_icons/ammo-box.png", "noclamp smooth")
...
SWEP.Supply_Logic = function(self)
	self.Owner:ChatPrint("Na dany moment zrzut jest niemożliwy!");
end
...
function SWEP:DrawHUD()
	...
	local _remap = math.Remap((self:GetNextPrimaryFire() - CurTime()) / self.Supply_Coldown, 0, 1, 1, 0)
	...
	surface.SetMaterial(self.Supply_icon)
    ...
    draw.ShadowSimpleText("LPM - Wywołanie zrzutu zapasów", "lunaMontMini", x, y, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM)
    draw.ShadowSimpleText("Bądź ostrożniejszy przy wyborze punktu! Nie zabijaj swoich.", "lunaMontMini", x, y+20, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM)
end
...
-- 	self:SetNextPrimaryFire(CurTime() + self.Supply_Coldown);
-- 	self.Supply_Logic(self);

-- 	local snd = self.Supply_Sound()
-- 	if snd and IsValid(self:GetOwner()) then
-- 		self:GetOwner():EmitSound(self.Supply_Sound(), 85, 100, 1, CHAN_WEAPON)
-- 	end
...
--     end )
--     ply:ChatPrint("Пилот СНДК: Посылка в пути, ожидайте.")
...
function SWEP:PrimaryAttack()
    ...
    ply:ChatPrint("Pilot WAR: Przesyłka w drodze, proszę czekać.")
    timer.Simple(1, function()
        net.Start("resupplyCallingInSound")
        net.Send(ply)
    end)
