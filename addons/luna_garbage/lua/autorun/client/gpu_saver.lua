...
/*
local convar = CreateClientConVar("gpu_saver", "1", true, false, "Настройки GPU заглушки", 0, 1)
local manual = false
local KeyboardController = NULL
local gpu_saver = Material('luna_ui_base/luna-ui_bg_var1.png', 'smooth noclamp')
...
do
	--surface.SetFont(luna.MontHUDPrimary)
	...
	local function GPUSaver()
		if not systemHasFocus() or manual then
			...
			local W, H = ScrW() * 0.44, ScrH() * 0.5

			camStart(t2D)

			surfaceSetDrawColor(0, 130, 255)
			surfaceDrawRect(0, 0, W * 2, H * 2)
			surface.SetMaterial(gpu_saver)
			surface.SetDrawColor(255, 255, 255, 255)
			surface.DrawTexturedRect(0, 0, ScrW(), ScrH())

			-- surfaceSetFont(luna.MontHUDPrimary)
