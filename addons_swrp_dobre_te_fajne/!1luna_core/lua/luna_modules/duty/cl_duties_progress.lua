...
local function DrawNextPatrolPoint(status)
	...
	if dist <= DUTIES_CONFIG.patrol_point_dist then
		...
		draw.ShadowSimpleText("Dotarłeś do punktu. Czekaj na instrukcje!", luna.MontBase18, icon_pos.x, icon_pos.y+icon_size*0.75, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	...
	elseif status == DUTY_PATROL_PREPARING or status == DUTY_STATION_PREPARING then
		...
		draw.ShadowSimpleText("Zbrojownia ("..dist_text.." м)", luna.MontBase18, icon_pos.x, icon_pos.y+icon_size*0.75, Color(17, 148, 240, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		draw.ShadowSimpleText("(odbierz broń)", luna.MontBase12, icon_pos.x, icon_pos.y+icon_size*1.1, Color(17, 148, 240, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		icon_mat = mat_armory

	elseif status == DUTY_PATROL_FINISH or status == DUTY_STATION_FINISH then

		draw.ShadowSimpleText("Zbrojownia ("..dist_text.." м)", luna.MontBase18, icon_pos.x, icon_pos.y+icon_size*0.75, Color(17, 148, 240, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		draw.ShadowSimpleText("(oddaj broń)", luna.MontBase12, icon_pos.x, icon_pos.y+icon_size*1.1, Color(17, 148, 240, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		icon_mat = mat_armory

	elseif status == DUTY_PATROL and fail_request_sent then

		draw.ShadowSimpleText("Punkt karny ("..dist_text.." м)", luna.MontBase18, icon_pos.x, icon_pos.y+icon_size*0.75, Color(240, 65, 17, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		draw.ShadowSimpleText("(naruszenie zasad patrolu)", luna.MontBase12, icon_pos.x, icon_pos.y+icon_size*1.1, Color(240, 65, 17, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)

	elseif status == DUTY_LOGISTICS then

		draw.ShadowSimpleText("Magazyn ("..dist_text.." м)", luna.MontBase18, icon_pos.x, icon_pos.y+icon_size*0.75, Color(17, 148, 240, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		draw.ShadowSimpleText("(odbierz pudełko)", luna.MontBase12, icon_pos.x, icon_pos.y+icon_size*1.1, Color(17, 148, 240, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		icon_mat = mat_box

	elseif status == DUTY_LOGISTICS_CARRY then

		draw.ShadowSimpleText("Odnieś pudełko ("..dist_text.." м)", luna.MontBase18, icon_pos.x, icon_pos.y+icon_size*0.75, Color(17, 148, 240, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		icon_mat = mat_task

	else
		draw.ShadowSimpleText("Przemieszczaj się do punktu ("..dist_text.." м)", luna.MontBase18, icon_pos.x, icon_pos.y+icon_size*0.75, Color(17, 148, 240, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	end
...
local function DrawStationPoint(point, id)
	...
	if dist <= DUTIES_CONFIG.patrol_point_dist then
		...
		icon_color = Color(17, 148, 240, 255)
		draw.ShadowSimpleText("Pozostań na miejscu, aby zająć stanowisko...", luna.MontBase18, icon_pos.x, icon_pos.y+icon_size*0.75, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		draw.Icon(icon_pos.x-icon_size*0.5,icon_pos.y-icon_size*0.5,icon_size,icon_size,icon_mat,icon_color)
		...
		if station_request_sent + 5 <= CurTime() then
			...
			draw.ShadowSimpleText("[#"..id.."] Wolny post ("..dist_text.." м)", luna.MontBase18, icon_pos.x, icon_pos.y+icon_size*0.75, Color(17, 148, 240, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
			draw.Icon(icon_pos.x-icon_size*0.5,icon_pos.y-icon_size*0.5,icon_size,icon_size,icon_mat,icon_color)
		end
...
net.Receive( "RE.Duty.Patrol_End", function( len )
	...
	if award then
		chat.AddText(Color(17, 148, 240), "SUP.Duty • ", Color(0, 200, 0), "Udało Ci się wykonać zadanie!")
...
net.Receive( "RE.Duty.Station_Select", function( len )
	...
	chat.AddText(Color(17, 148, 240), "SUP.Duty • ", Color(255, 255, 255), "Wstąpiłeś na ", Color(17, 148, 240), "Post #"..id, Color(255, 255, 255), ". Nie opuszczaj go do końca czasu.")
	surface.PlaySound("luna_sound_effects/info/infoobnovleno4.mp3")
end )
...
hook.Add( "HUDPaint", "HUD_Duty_Active", function()
	...
	if status == DUTY_PATROL and LocalPlayer():IsSprinting() and not fail_request_sent then
		...
		chat.AddText(Color(17, 148, 240), "SUP.Duty • ", Color(240, 65, 17), "Ruch podczas patrolu z biegiem jest zabroniony! Ten punkt nie będzie zaliczony.")
		surface.PlaySound("luna_characters/c_menu/heyyouthere.mp3")
	end

	if status == DUTY_STATION_CHOOSING then
		tasks[1] = "Wybierz jeden z postów i stań na nim"
		tasks[2] = "Pozostań na miejscu, aby zająć wybrany post"
	end
	if status == DUTY_STATION_PREPARING or status == DUTY_STATION_CHOOSING or status == DUTY_STATION_FINISH then
		DrawDutyStatus("Służba posterunkowa", DUTY_STATUS_TEXTS[status], tasks, re_duty_end_time, true)
	end
	if status == DUTY_STATION then
		DrawActiveStation()
		
		if fail_timer == 0 then
			tasks[1] = "Nie opuszczaj granic posterunku do końca czasu"
			tasks[2] = "Trzymaj zabezpieczoną broń w rękach"
			DrawDutyStatus("Służba posterunkowa", DUTY_STATUS_TEXTS[status], tasks, re_duty_end_time)
		else
			tasks[1] = "Wracajcie na posterunek! Zostało "..math.Round(end_timer).." sek"
			DrawDutyStatus("Służba posterunkowa", DUTY_STATUS_TEXTS[status], tasks, fail_timer, true)
		end
	end

	if status >= DUTY_PATROL_PREPARING and status <= DUTY_PATROL_FINISH then
		if status == DUTY_PATROL then
			tasks[1] = "Przebyte punkty patrolowe: ".. (re_duty_patrol_progress or 0) .. "/" .. DUTIES_CONFIG.patrol_num_points
			tasks[2] = "Poruszaj się tylko krokiem z zabezpieczoną bronią"
		end
		DrawDutyStatus("Patrolowanie", DUTY_STATUS_TEXTS[status], tasks)
	elseif status == DUTY_LOGISTICS or status == DUTY_LOGISTICS_CARRY then
		tasks[1] = "Przeniesione pudełka: ".. (re_duty_patrol_progress or 0) .. "/" .. DUTIES_CONFIG.logistics_minimum
		DrawDutyStatus("Logistyka", DUTY_STATUS_TEXTS[status], tasks)
	end
end )
