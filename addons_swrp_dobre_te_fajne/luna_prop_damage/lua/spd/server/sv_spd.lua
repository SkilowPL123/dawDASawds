...
function spdEntityTakeDamage(ent, dmg)
	...
	if ent.spdDisabled ~= false then return end
	if GetConVar("spd_physicsdamage"):GetInt() == 0 and dmg:IsDamageType(DMG_CRUSH) then return end
	if GetConVar("spd_bulletdamage"):GetInt() == 0 and dmg:IsDamageType(DMG_BULLET) then return end
	if GetConVar("spd_explosiondamage"):GetInt() == 0 and dmg:IsDamageType(DMG_BLAST) then return end
...
function spdDebris(ent)
	if GetConVar("spd_debris"):GetInt() == 0 then return end
