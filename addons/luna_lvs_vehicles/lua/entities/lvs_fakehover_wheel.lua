...
function ENT:Think()
    ...
    -- Увеличиваем интервал для проверки создания эффекта до 0.1 секунд
    if (self._nextFX or 0) > T then return end
    self._nextFX = T + 0.1  -- увеличено с 0.02 до 0.1

    local base = self:GetBase()

    if not IsValid(base) then return end

    -- Проверяем скорость движения, чтобы избежать лишних эффектов на малых скоростях
    if base:GetVelocity():Length() < 100 then return end  -- увеличено значение порога скорости
