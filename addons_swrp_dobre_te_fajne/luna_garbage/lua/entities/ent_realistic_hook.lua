...
function ENT:PhysicsCollide( data, phys )
	...
	if not IsValid(self:GetWep()) and IsValid(self:GetWep():GetOwner()) and self:GetWep():GetOwner():IsPlayer() then self:Destroyed( true ) return end
	...
	timer.Simple(0,function()
		...
		if not (IsValid(self:GetWep()) and IsValid(self:GetWep():GetOwner())) then self:Destroyed( true ) return end
		...
		self:SetDist( data.HitPos:Distance( self:GetWep():GetOwner():GetShootPos() ) )
...
function ENT:HUDPaint()
	...
	ShadowText( "Długość: " ..tostring(self:GetDist()), luna.LunaMontMini, ScrW()/2, ScrH()/2-60 )
	ShadowText( "Użyto na was haczyka!", luna.LunaMontMini, ScrW()/2, ScrH()/2-75, Color(255,100,100) )
	ShadowText( (input.LookupBinding("+use") or "[USE]"):upper() .. " - Uwolnij się", luna.LunaMontMini, ScrW()/2, ScrH()/2-40 )
