--leak by matveicher
--vk group - https://vk.com/codespill
--steam - https://steamcommunity.com/profiles/76561198968457747/
--ds server - https://discord.gg/7XaRzQSZ45
--ds - matveicher

att.PrintName = "Барабанный Магазин"
att.AbbrevName = "100-Round Mag"
att.SortOrder = 100
att.Icon = nil
att.Description = "Двухбарабанный магазин, идеально подходящий для ведения огня на подавление. Громоздкая конструкция и большая емкость боеприпасов существенно увеличивают вес оружия, делая его трудноуправляемым. Склонность к отказам."
att.Desc_Pros = {}
att.Desc_Cons = {}

att.Desc_Neutrals = {}
att.Slot = "dc15a_magazine_100"

att.Model = "models/weapons/arccw/atts/magazine/rt97_rifle_magazine.mdl"
att.DroppedModel = "models/Items/BoxSRounds.mdl"
att.OffsetAng = Angle(0, 0, -90)

att.AutoStats = true

att.HideIfBlocked = true

att.Override_ClipSize = 100

att.Mult_SightTime = 1.4
att.Mult_ReloadTime = 1.7
att.Mult_Sway = 2.1

att.Mult_SpeedMult = 0.9
att.Mult_ShootSpeedMult = 0.8

att.Mult_DrawTime = 1.25
att.Mult_HolsterTime = 1.25

att.Mult_HipDispersion = 1.3

--[[]
att.Override_Jamming = true
att.Override_HeatCapacity = 200
att.Override_HeatDissipation = 4
att.Override_HeatDelayTime = 3
]]


--leak by matveicher
--vk group - https://vk.com/codespill
--steam - https://steamcommunity.com/profiles/76561198968457747/
--ds server - https://discord.gg/7XaRzQSZ45
--ds - matveicher
