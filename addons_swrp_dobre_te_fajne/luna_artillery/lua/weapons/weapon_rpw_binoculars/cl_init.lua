--leak by matveicher
--vk group - https://vk.com/codespill
--steam - https://steamcommunity.com/profiles/76561198968457747/
--ds server - https://discord.gg/7XaRzQSZ45
--ds - matveicher

include( "shared.lua" )

-- language.Add("weapon_rpw_binoculars", "Binoculars")

-- surface.CreateFont( "rangefinder", {
-- 	font = "TargetID",
-- 	extended = false,
-- 	size = 32,
-- 	weight = 600,
-- 	blursize = 0,
-- 	scanlines = 0,
-- 	antialias = true,
-- 	underline = false,
-- 	italic = false,
-- 	strikeout = false,
-- 	symbol = false,
-- 	rotary = false,
-- 	shadow = false,
-- 	additive = false,
-- 	outline = false,
-- } )

SWEP.PrintName 				= "Artyleria AV7"
SWEP.Slot 					= 0
SWEP.SlotPos 				= 1
SWEP.DrawAmmo 				= false
SWEP.DrawCrosshair 			= false
SWEP.ViewModelFlip 			= false

SWEP.WepSelectIcon 			= surface.GetTextureID("vgui/swepicons/weapon_rpw_binoculars")
SWEP.DrawWeaponInfoBox		= false
SWEP.BounceWeaponIcon 		= false 

SWEP.ViewModelFOV			= 65

function SWEP:ShouldDrawViewModel()
	return false
end

function SWEP:DrawHUD()
	if (self.Zoom_InZoom) then
		local mat_bino_overlay = Material( "kaito/bino_overlay.png" )
		local w = ScrW()
		local h = ScrH()
		surface.SetMaterial( mat_bino_overlay )
        surface.SetDrawColor(0, 0, 0, 255)
        surface.DrawTexturedRect(0, 0, ScrW(), ScrH())
		
		local tr = self.Owner:GetEyeTrace().HitPos

		local x = math.Truncate(tr.x, 1)
		local y = math.Truncate(tr.y, 1)

		surface.SetFont( luna.MontBase45 )
		surface.SetTextColor( 255, 255, 255, 255 )
		surface.SetTextPos( (w*0.165), (h/2) + 16 )
		surface.DrawText( "X: "..x.." Y: "..y )
		
		local zoom = math.ceil(90/self.Owner:GetFOV())
		
		surface.SetTextPos( (w*0.775), (h/2) + 16 )
		surface.DrawText( "Odległość: "..zoom.."x" )
	end
end

function SWEP:AdjustMouseSensitivity()
	if (self.Zoom_InZoom) then
		local zoom = 90/self.Owner:GetFOV()
		local adjustedsens = 1 / zoom
		return adjustedsens
	end
end




--leak by matveicher
--vk group - https://vk.com/codespill
--steam - https://steamcommunity.com/profiles/76561198968457747/
--ds server - https://discord.gg/7XaRzQSZ45
--ds - matveicher
