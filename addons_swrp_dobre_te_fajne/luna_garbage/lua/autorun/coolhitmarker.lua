...
hook.Add("PlayerSwitchFlashlight", "FlashlightRestriction", function(ply, state)
    -- Получаем индекс bodygroup "flashlighthelmet"
    local bodygroupIndex = ply:GetBodygroup(5) -- Предполагается, что bodygroup "flashlighthelmet" имеет индекс 1
    
    -- Проверяем, является ли bodygroup "flashlighthelmet" активным (значение bodygroup не равно 0)
    if IsValid(ply) and bodygroupIndex == 1 then
        -- Если bodygroup активен, позволяем игроку использовать фонарик
        return true
    else
        -- Если bodygroup отсутствует или не активен, запрещаем использование фонарика
        return false
    end
end)
