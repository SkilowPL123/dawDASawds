...
setmetatable(patrolPoint, {
	__call = function(c, ...) return c.create(...) end
})
...
function patrolPoint:GetRenderBounds()
	if self.myModel then return self.myModel:GetRenderBounds() end
end
...
function patrolPoint:SetID(ID)
	self.pr_id = ID
end

function patrolPoint:GetID()
	return self.pr_id
end

function patrolPoint:SetWait(wait)
	self.pr_wait = wait
end

function patrolPoint:GetWait()
	return self.pr_wait
end
...
function patrolPoint:SetOrigin(pos)
	self.pr_origin = pos
	self:SetPos(pos)
end

function patrolPoint:SetVisible(b)
	self.pr_visible = b
end
...
hook.Add("Think", "patrolThink", function()
	for i = #PATROL_POINTS, 1, -1 do
		...
		if v then
			if v.Think and not v:Think() then
				if v.OnRemove then
					v:OnRemove()
				end
...
function patrolPoint:Render()
	if not self.pr_visible then return end
	self:DrawModel()
	local id = self:GetID()
	...
	if id then
		...
		if wait and wait > 0 then
			draw.DrawText("Wait: " .. wait, "default", 0, 10, colText, TEXT_ALIGN_CENTER)
		end
