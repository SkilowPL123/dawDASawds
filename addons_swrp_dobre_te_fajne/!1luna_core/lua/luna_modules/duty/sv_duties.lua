...
local function RegisterDutyLeader(ply, type, result)
	...
	if result > cur_result then
		...
		local type_name = "zadaniom"
		if type == 1 then
			type_name = "patrolom"
		elseif type == 2 then
			type_name = "posterunkom"
		elseif type == 3 then
			type_name = "logistyce"
		end
		ChatAddTextAll(Color(17, 148, 240), "Piwnica.duty • ", Color(255, 255, 255), "Żołnierz ", Color(17, 148, 240), nick, Color(255, 255, 255), " stał się liderem w zakresie ", Color(17, 148, 240), type_name, Color(255, 255, 255), " z wynikiem ", Color(17, 148, 240), tostring(result))
	end
end
...
local function AddDutyStat(ply, type, amount)
	...
	local average = 0
	local message = "[Piwnica.duty] "..name.." ("..steamID..") wykonał zadanie "..type.." w "..time.." sekund."
	if type == 1 then
		average = time / (DUTIES_CONFIG.patrol_num_points + 2)
		message = message.." Średni czas między punktami: ".. average
	elseif type == 3 then
		average = time / amount
		message = message.." Średni czas na skrzynkę: ".. average
	end
...
local function SendToNextPickup(ply, storage)
	...
	if storage then
		...
		point_name = point[2]
		point = point[1]
		ChatAddText(ply, Color(17, 148, 240), "Piwnica.duty • ", Color(255, 255, 255), "Idź do miejsca rozładunku: ", Color(17, 148, 240), point_name)
	end
...
local function DestinationDropBox(ply)
	...
	ChatAddText(ply, Color(17, 148, 240), "Piwnica.duty • ", Color(255, 255, 255), "Pomyślnie dostarczyłeś paczkę. Wracaj do magazynu!")
	if progress == min then
		ChatAddText(ply, Color(17, 148, 240), "Piwnica.duty • ", Color(255, 255, 255), "Wykonałeś normę ", Color(0, 200, 0), min.." paczek", Color(255, 255, 255),". Możesz kontynuować pracę lub odebrać nagrodę przy terminalu.")
	elseif progress < min then
		ChatAddText(ply, Color(17, 148, 240), "Piwnica.duty • ", Color(255, 255, 255), "Przenieś jeszcze ", Color(17, 148, 240), min-progress.." paczek", Color(255, 255, 255),", aby otrzymać nagrodę!")
	end
end
...
local function ArmoryPickup(ply)
	...
	if status == DUTY_PATROL_PREPARING then
		...
		ChatAddText(ply, Color(17, 148, 240), "Piwnica.duty • ", Color(255, 255, 255), "Twoim zadaniem jest poruszanie się po punktach patrolowych.")
		ChatAddText(ply, Color(17, 148, 240), "• ", Color(240, 65, 17), "Poruszaj się tylko marszem, trzymaj broń w rękach zabezpieczoną.")
		ply.Duty_Progress = 0

		SendNextPatrolPoint(ply)
	elseif status == DUTY_STATION_PREPARING then
		ply:SetNW2Int("Duty_Status", DUTY_STATION_CHOOSING)
		ChatAddText(ply, Color(17, 148, 240), "Piwnica.duty • ", Color(255, 255, 255), "Wybierz jeden z wolnych posterunków.")
		ChatAddText(ply, Color(17, 148, 240), "• ", Color(255, 255, 255), "Aby wybrać posterunek - pozostań na miejscu.")
		ply.Duty_Progress = 0
...
local function GivePatrolAward(ply)
	...
	ChatAddText(ply, Color(17, 148, 240), "Piwnica.duty • ", Color(255, 255, 255), "Otrzymałeś ", Color(17, 148, 240), money.."KR", Color(255, 255, 255), " i ", Color(17, 148, 240), exp.."EXP", Color(255, 255, 255), " za patrol.")
	ChatAddText(ply, Color(17, 148, 240), "Piwnica.duty • ", Color(255, 255, 255), "W tej chwili system jest konfigurowany. Po teście nagrody mogą zostać zmienione!")
	ply:AddMoney(money)
...
local function GiveStationAward(ply)
	...
	ChatAddText(ply, Color(17, 148, 240), "Piwnica.duty • ", Color(255, 255, 255), "Otrzymałeś ", Color(17, 148, 240), money.."KR", Color(255, 255, 255), " i ", Color(17, 148, 240), exp.."EXP", Color(255, 255, 255), " za post.")
	ChatAddText(ply, Color(17, 148, 240), "Piwnica.duty • ", Color(255, 255, 255), "W tej chwili system jest konfigurowany. Po teście nagrody mogą zostać zmienione!")
	ply:AddMoney(money)
	ply:AddCharExperience(exp)
...
local function GiveLogisticsAward(ply, progress)
	ChatAddText(ply, Color(17, 148, 240), "Piwnica.duty • ", Color(255, 255, 255), "Otrzymałeś nagrodę za ", Color(17, 148, 240), progress.." pudełek", Color(255, 255, 255),"! Tak trzymaj!")

	
	local money = progress * DUTIES_CONFIG.logistics_award_money
	local exp = progress * DUTIES_CONFIG.logistics_award_exp

	ChatAddText(ply, Color(17, 148, 240), "Piwnica.duty • ", Color(255, 255, 255), "Otrzymałeś ", Color(17, 148, 240), money.." KR", Color(255, 255, 255), " i ", Color(17, 148, 240), exp.." EXP", Color(255, 255, 255), " za przeniesione ładunki: ", Color(17, 148, 240), progress)
	ChatAddText(ply, Color(17, 148, 240), "Piwnica.duty • ", Color(255, 255, 255), "W tej chwili system jest konfigurowany. Po teście nagrody mogą zostać zmienione!")
	ply:AddMoney(money)
	ply:AddCharExperience(exp)
...
local function EndPatrol(ply)
	if not IsOnDuty(ply) then return "Nie jesteś na patrolu" end
	...
	if progress > DUTIES_CONFIG.patrol_num_points then
		...
		ChatAddText(ply, Color(17, 148, 240), "Piwnica.duty • ", Color(240, 65, 17), "Wykonanie zadania zostało anulowane!")
	end
...
local function EndStation(ply)
    local status = ply:GetNW2Int("Duty_Status", 0)
    if status < DUTY_STATION_PREPARING or status > DUTY_STATION_FINISH then return "Nie jesteś na postowej służbie" end
...
local function EndLogistics(ply)
	if not IsOnDuty(ply) then return "Nie pracujesz" end
...
hook.Add( "PlayerSay", "RE.Duty", function( ply, text )
	if ( string.lower( text ) == "/patrol" ) then
		...
		if check then
			...
			ChatAddText(ply, Color(17, 148, 240), "Piwnica.Duty • ", Color(240, 65, 17), msg)
		end
...
net.Receive("RE.Duty.Station_Select", function( len, ply )
	...
	if IsValid(occupant) then
		if occupant.Duty_Station != point_id then
			...
			ChatAddText(ply, Color(17, 148, 240), "Piwnica.Duty • ", Color(240, 65, 17), "To stanowisko jest już zajęte. Wybierz inne!")
			SendNextPatrolPoint(ply)
			return false
		end
	end
	...
	timer.Create("Duty_Station_"..ply:SteamID64(), DUTIES_CONFIG.station_wait, 1, function()
		if IsValid(ply) then
			...
			ChatAddText(ply, Color(17, 148, 240), "Piwnica.Duty • ", Color(255, 255, 255), "Pomyślnie przetrwałeś służbę. Udaj się do punktu oddania broni!")
			timer.Adjust("Duty_Station_"..ply:SteamID64(), DUTIES_CONFIG.station_equip_time, 1, function() EndStation(ply) end)
		end
	end)
end )
...
function RE_Duty_Patrol_Init(ply)
	...
	ChatAddText(ply, Color(17, 148, 240), "Piwnica.Duty • ", Color(255, 255, 255), "Wychodzisz na patrol.")
	ChatAddText(ply, Color(17, 148, 240), "• ", Color(255, 255, 255), "Udaj się do arsenału, aby otrzymać broń.")
...
function RE_Duty_Station_Init(ply)
	...
	ChatAddText(ply, Color(17, 148, 240), "Piwnica.Duty • ", Color(255, 255, 255), "Wychodzisz na służbę na posterunku.")
	ChatAddText(ply, Color(17, 148, 240), "• ", Color(255, 255, 255), "Udaj się do arsenału, aby otrzymać broń.")
...
function RE_Duty_Logistics_Init(ply)
	...
	ChatAddText(ply, Color(17, 148, 240), "Piwnica.Duty • ", Color(255, 255, 255), "Rozpocząłeś pracę przy przenoszeniu materiałów.")
	ChatAddText(ply, Color(17, 148, 240), "• ", Color(255, 255, 255), "Udaj się do magazynu, odbierz pudełko i przynieś je tam, gdzie zostaniesz poproszony.")
...
local function Request_Duty(ply, duty_type)
	...
	if ply:GetMoveType() == MOVETYPE_NOCLIP then 
		ChatAddText(ply, Color(17, 148, 240), "Piwnica.Duty • ", Color(240, 65, 17), "Odsuń się! Jesteś w trybie lotu!")
		return 
	end
...
hook.Add("KeyPress", "CheckPlayerJumping", function(ply, key)
	if key == IN_JUMP and ply.bWearBox then
		...
		ChatAddText(ply, Color(17, 148, 240), "Piwnica.Duty • ", Color(240, 65, 17), "Upuściłeś pudełko!")
	end
end)
