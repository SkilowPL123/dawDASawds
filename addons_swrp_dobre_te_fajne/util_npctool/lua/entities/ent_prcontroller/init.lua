...
function ENT:GetNextPoint()
    ...
    elseif self.pr_links[self.pr_currentnode][1][1] then
        return self.pr_links[self.pr_currentnode][1][1][1]
    else
        return -1
    end
end
...
function ENT:GetPrevPoint()
    ...
    elseif self.pr_links[self.pr_currentnode][2][1] then
        return self.pr_links[self.pr_currentnode][2][1][1]
    else
        return -1
    end
end
...
local idle = {ACT_IDLE, ACT_IDLE_STEALTH, ACT_IDLE_ANGRY_MELEE, ACT_IDLE_RELAXED, ACT_IDLE_SMG1_RELAXED, ACT_IDLE_AIM_STEALTH, ACT_IDLE_MANNEDGUN, ACT_IDLE_ANGRY_PISTOL, ACT_IDLE_SHOTGUN_RELAXED, ACT_IDLE_RIFLE, ACT_IDLE_AIM_RELAXED, ACT_IDLE_ON_FIRE, ACT_IDLE_SHOTGUN_AGITATED, ACT_IDLE_ANGRY_SMG1, ACT_IDLE_PISTOL, ACT_IDLE_AIM_RIFLE_STIMULATED, ACT_IDLE_AGITATED, ACT_IDLE_CARRY, ACT_IDLE_RPG, ACT_IDLE_HURT, ACT_IDLE_ANGRY_RPG, ACT_IDLE_ANGRY_SHOTGUN, ACT_IDLE_SUITCASE, ACT_IDLE_SMG1_STIMULATED, ACT_IDLE_MELEE, ACT_IDLE_ANGRY, ACT_IDLE_AIM_STIMULATED, ACT_IDLE_AIM_AGITATED, ACT_IDLE_PACKAGE, ACT_IDLE_STEALTH_PISTOL, ACT_IDLE_SMG1, ACT_IDLE_RPG_RELAXED, ACT_IDLE_SHOTGUN_STIMULATED, ACT_IDLE_STIMULATED}
...
function ENT:Think()
    if not IsValid(self.pr_NPC) and IsValid(self.entpos) then
    ...
    if IsValid(self.pr_NPC) and self.pr_NPC:Health() > 0 and self.pr_currentnode > 0 then
        ...
        --end
        -----------
        if (self.GetState and ent:GetState() or ent:GetNPCState() <= NPC_STATE_ALERT and not ent:HasCondition(7)) or self:GetStrictMovement() then
            ...
            if self.pr_NPC:GetClass() == "npc_strider" then
                compdist = 500
            end
            ...
            if dist <= compdist then
                if self.pr_nodes[self.pr_currentnode][2] ~= 0 and self.waitdone == false then
                    ...
                    timer.Simple(self.pr_nodes[self.pr_currentnode][2], function()
                        self.waiting = false
                    end)
                end
                ...
                if (table.HasValue(idle, act) or (act == ACT_WALK and not ent:IsCurrentSchedule(SCHED_FORCED_GO))) and (not ent.sm_investigating or ent.sm_investigating == 0) then
                    ent:SetLastPosition(pos + Vector(0, 0, 6))
                    local schd

                    if self:GetWalk() then
                        schd = SCHED_FORCED_GO
                    else
                        schd = SCHED_FORCED_GO_RUN
                    end
