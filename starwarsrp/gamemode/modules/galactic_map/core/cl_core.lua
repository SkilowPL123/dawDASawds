...
function GMap:Menu()
    ...
    cloneWar.Paint = function(_, w, h)
        ...
        draw.SimpleText('WOJNA KLONÓW', 'gmap.1', scale(10), scale(5), cfg.colors.white, 0, 3)
        ...
        draw.SimpleText('Planety sektora: ' .. table.Count(GALACTIC_MAP), 'gmap.2', scale(10), scale(75), cfg.colors.white, 0, 3)
        draw.markupText({
            text = {
                {
                    text = 'Separatyści: ',
    ...
    infoPanel.Paint = function(_, w, h)
        draw.Blur(_, 3)
        draw.NewRect(0, 0, w, h, cfg.colors.main_back)
        draw.SimpleText('INFORMACJE', 'gmap.1', w * 0.5, scale(10), cfg.colors.white, 1, 3)
        ...
        draw.SimpleText('«PLANETA ' .. utf8.upper(planetName) .. '»', 'gmap.3', w * 0.5, nameY, cfg.colors.blue, 1, 3)
        lineY = fr.descInfo:GetY() + fr.descInfo:GetTall() + margin
        draw.NewRect(lineMargin, lineY, w - lineMargin * 2, lineHeight, cfg.colors.white)
        local warInfoY = lineY + margin
        draw.SimpleText('«INFORMACJE WOJENNE»', 'gmap.3', w * 0.5, warInfoY, cfg.colors.blue, 1, 3)
        lineY = fr.orderPanel:GetY() + fr.orderPanel:GetTall() + margin * 2
        draw.NewRect(lineMargin, lineY, w - lineMargin * 2, lineHeight, cfg.colors.white)
        local progressY = lineY + margin
        draw.SimpleText('«AKTUALNY POSTĘP»', 'gmap.3', w * 0.5, progressY, cfg.colors.blue, 1, 3)
    ...
    orderWar.Paint = function(_, w, h)
        draw.Blur(_, 3)
        draw.NewRect(0, 0, w, h, cfg.colors.main_back)
        draw.SimpleText('ROZKAZ SZTABU', 'gmap.1', w * 0.5, scale(10), cfg.colors.white, 1, 3)
        local lineY = headerHeight
        draw.NewRect(lineMargin, lineY, w - lineMargin * 2, lineHeight, cfg.colors.white)
        local nameY = lineY + margin
    ...
    companyInfo.Paint = function(_, w, h)
        draw.Blur(_, 3)
        draw.NewRect(0, 0, w, h, cfg.colors.main_back)
        draw.SimpleText('AKTUALNA KAMPANIA', 'gmap.1', w * 0.5, scale(10), cfg.colors.white, 1, 3)
        local lineY = headerHeight
        draw.NewRect(lineMargin, lineY, w - lineMargin * 2, lineHeight, cfg.colors.white)
        local nameY = lineY + margin
        local warInfo = GMap.Planets
    ...
    local function UpdatePlanetInfo(planetIndex)
        ...
        warnInfo:SetText('Aby przejąć kontrolę nad planetą «' .. planetInfo.name .. '», trzeba:')
