...
net.Receive( "awarn_fetchwarnings", function( l, ply )
	...
	if not awarn_checkadmin_view( ply ) then
		AWSendMessage( ply, "SUP.Warns • " .. AWarn.localizations[loc].sv1)
		return
	end
	...
	if player_type == "playername" then
		...
		if target_ply then
			...
			AWSendMessage( ply, "SUP.Warns • " .. AWarn.localizations[loc].cl37)
		end
...
net.Receive( "awarn_changeconvarbool", function( l, ply )
	...
	if not table.HasValue( allowed, convar ) then
		AWSendMessage( ply, "SUP.Warns • " .. AWarn.localizations[loc].sv2)
		return
	end
...
net.Receive( "awarn_changeconvar", function( l, ply )
	...
	if val < 0 then
		AWSendMessage( ply, "SUP.Warns • " .. AWarn.localizations[loc].sv3)
		return
	end
...
function awarn_con_warn( ply, _, args )
	...
	if (string.sub(string.lower(args[1]), 1, 5) == "steam") then
		if string.len(args[1]) == 7 then
			AWSendMessage( ply, "SUP.Warns • " .. AWarn.localizations[loc].cl36)
			return
		end
...
function awarn_warnplayer( ply, tar, reason )
	...
	if table.HasValue( AWarn.userBlacklist, tar:SteamID() ) then
		AWSendMessage( ply, "SUP.Warns • " .. AWarn.localizations[loc].sv9)
		return
	end
	
	if table.HasValue( AWarn.groupBlacklist, tar:GetUserGroup() ) then
		AWSendMessage( ply, "SUP.Warns • " .. AWarn.localizations[loc].sv10)
		return
	end
	
	if awarn_checkadmin_warn( tar ) and not GetConVar("awarn_allow_warnadmin"):GetBool() then
		AWSendMessage( ply, "SUP.Warns • " .. AWarn.localizations[loc].sv4)
		return
	end
	...
	if GetConVar("awarn_reasonrequired"):GetBool() then
		if not reason then
			AWSendMessage( ply, "SUP.Warns • " .. AWarn.localizations[loc].sv5)
			return
		end
		if reason == "" then
			AWSendMessage( ply, "SUP.Warns • " .. AWarn.localizations[loc].sv5)
	...
	if target_ply then
		...
		if IsValid(ply) then
            ...
            ServerLog( "SUP.Warns " .. ply:Nick() .. " warned " .. target_ply:Nick() .. " z powodu: " .. reason.. "\n" )
			if GetConVar("awarn_logging"):GetBool() then
				awarn_log( ply:Nick() .. " warned " .. target_ply:Nick() .. " z powodu: " .. reason )
			end
        else
            awarn_addwarning( target_ply:SteamID64(), reason, "[CONSOLE]" )
            ServerLog( "SUP.Warns [SUP.System] ostrzeżony " .. target_ply:Nick() .. " z powodu: " .. reason.. "\n" )
			if GetConVar("awarn_logging"):GetBool() then
				awarn_log( "[SUP.System] ostrzeżony " .. target_ply:Nick() .. " z powodu: " .. reason )
			end
        end
		...
		local t1 = {}
        if IsValid( ply ) then
            t1 = { Color(17, 148, 240), "SUP.Warns • ", Color(255,255,255), AWarn.localizations[loc].sv6 .. " ", ply, ": ", Color(150,40,40), reason }
        else
            t1 = { Color(17, 148, 240), "SUP.Warns • ", Color(255,255,255), AWarn.localizations[loc].sv6 .. " ", Color(100,100,100), "[CONSOLE]", Color(255,255,255), ": ", Color(150,40,40), reason }
        end
		net.Start("AWarnChatMessage") net.WriteTable(t1) net.Send( target_ply )
		
		local t5 = { Color(17, 148, 240), "SUP.Warns • ", Color(255,255,255), AWarn.localizations[loc].sv7 }
...
function awarn_warnplayerid( ply, tarid, reason )
	...
	if IsValid(ply) then
		...
		ServerLog( "SUP.Warns " .. ply:Nick() .. " warned " .. tostring(tar_name) .. " z powodu: " .. reason.. "\n" )
		if GetConVar("awarn_logging"):GetBool() then
			awarn_log( ply:Nick() .. " warned " .. tostring(tar_name) .. " z powodu: " .. reason )
		end
	else
		awarn_addwarning( tarid, reason, "[CONSOLE]" )
		ServerLog( "SUP.Warns [SUP.System] ostrzeżony " .. tostring(tar_name) .. " z powodu: " .. reason.. "\n" )
		if GetConVar("awarn_logging"):GetBool() then
			awarn_log( "[SUP.System] ostrzeżony " .. tostring(tar_name) .. " z powodu: " .. reason )
		end
	end
...
function awarn_remwarn( ply, tar )
	...
	if target_ply then
		...
		AWSendMessage( ply, "SUP.Warns • Player not found!")
	end
...
net.Receive( "awarn_openmenu", function( l, ply )
    ...
    if not IsValid( ply ) then
		AWSendMessage( ply, "SUP.Warns • " .. AWarn.localizations[loc].sv8)
        return
    end
...
for k, v in pairs( files ) do
	ServerLog("SUP.Warns • Loading module (" .. v .. ")\n")
	include( "awarn/modules/" .. v )
end

hook.Add("PlayerSay", "lockcleanup", function(ply, txt)
    local cmd = string.lower(txt)  
    if cmd == "!cleanup" or cmd == "gmod_admin_cleanup" then
        if ply:IsAdmin() then
            ply:ChatPrint("")
            return ""
        end
...
hook.Add("CanTool", "lockcleanuptool", function(ply, tool)
    if tool == "clean_up" and ply:IsAdmin() then
        ply:ChatPrint("")
        return false
    end
end)
