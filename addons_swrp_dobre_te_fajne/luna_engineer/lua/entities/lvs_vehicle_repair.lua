...
if SERVER then
	...
	function ENT:Initialize()	
		self:SetModel( "models/battleground/droids/r1_astromech.mdl" )
	...
	function ENT:Refil( entity )
		...
		if entity:GetHP() ~= entity:GetMaxHP() then
			...
			entity:EmitSound("battleground/droids/r1_astromech_droid_repair_4.wav")
		end

		if entity:WeaponRestoreAmmo() then
			entity:EmitSound("battleground/droids/r1_astromech_droid_repair_1.wav")
		end
...
if CLIENT then
	...
	local RepairMat = Material( "luna_icons/tinker.png" )
	function ENT:Draw()
		...
		for i = 0, 180, 180 do
			cam.Start3D2D( self:LocalToWorld( Vector(0,0, self:OBBMins().z + 2 ) ), self:LocalToWorldAngles( Angle(i,90,0) ), 0.25 )
				...
				if Small then
					surface.DrawTexturedRect( -126, 170, 256, 256 )
				else
					surface.DrawTexturedRect( -512, -512, 1024, 1024 )
				end
			cam.End3D2D()
