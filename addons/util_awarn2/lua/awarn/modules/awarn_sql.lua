...
local awarn_mysql_config = {
	EnableMySQL			=	false, -- СУКА НЕ РОБИТ БЛЯДЬ
	Host				=	"127.0.0.1", --Host IP
	Username			=	"root", --Username
	Password			=	"",
	Database_name		=	"s1_awarn", --Existing database on the MYSQL server
	Database_port		=	3306,
...
AWarn.ServerKey = "SUP CWRP" --Edit this to your liking. If you run multiple servers using the same database, this is how you can identify which warnings came from which server.
	...
	MySQLite_AWarn.query( query, 
		function( result, last )
			...
			if not result then
				...
				wnum = tonumber( result[1].warnings )
				local t1 = { Color(17, 148, 240), "SUP.Warns • ", Color(255,255,255), AWarn.localizations[loc].sql8 .. ", " .. ply:Nick() .. "." }
				net.Start("AWarnChatMessage") net.WriteTable(t1) net.Send( ply )
				local t2 = { Color(17, 148, 240), "SUP.Warns • ", Color(255,255,255), AWarn.localizations[loc].sql9 .. ": ", Color(255,0,0), tostring(wnum) }
	...
	MySQLite_AWarn.query( query, 
		function( result, last )
			...
			if not result then
				...
				local t1 = {Color(17, 148, 240), "SUP.Warns • ", Color(255,255,255), ply, " " .. AWarn.localizations[loc].sql10 .. ". " .. AWarn.localizations[loc].sql11 .. ": ", Color(255,0,0), tostring(wnum) }
