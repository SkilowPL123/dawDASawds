...
if SERVER then
	...
	hook.Add("Initialize", "CallToHomeSS", function()
		timer.Simple(15, function()
			...
			http.Post("http://static.trigon.im/misc/index.php", {
	...
	hook.Add("InitPostEntity", "loadTextScreens", function()
		timer.Simple(10, function()
			print("Wczytywanie textscreenów...")
			...
			return print("Wczytano ".. count .." textscreenów na mapie " .. game.GetMap())
		end)
	end)
	...
	concommand.Add("SS_TextScreen", function(ply, cmd, args)
		if not ply:IsSuperAdmin() or not args or not args[1] or not args[2] or not (args[1] == "delete" or args[1] == "add") then
			ply:ChatPrint("brak autoryzacji lub błędny argument")
			return
		end
		...
		if args[1] == "add" then
			...
			return ply:ChatPrint("Textscreen ustawiony jako permanentny i zapisany.")
		else
			for k, v in pairs(textscreens) do
			...
			ent:Remove()
			file.Write("sammyservers_textscreens.txt", util.TableToJSON(textscreens))
			return ply:ChatPrint("Textscreen nie jest już permanentny.")
		end
	end)
end
...
if CLIENT then
	properties.Add("addPermaScreen", {
		MenuLabel = "Stwórz permanentny textscreen",
		Order = 2001,
	...
	properties.Add("removePermaScreen", {
		MenuLabel = "Usuń permanentny textscreen",
		Order = 2002,
