...
luna.observer.dimDistance = 1024
luna.observer.fullbright = true
luna.observer.enabled = true
luna.observer.player = true
luna.observer.blur = true
luna.observer.steam = true

net.Receive("lunaObserverFlashlight", function(len, ply)
	LocalPlayer():EmitSound("buttons/lightswitch2.wav")
end)

hook.Add("DrawPhysgunBeam", "luna.Observer-DrawPhysgunBeam", function(client, physgun, enabled, target, bone, hitPos)
    if (client != LocalPlayer() and client:GetMoveType() == MOVETYPE_NOCLIP) then
		return false
	end
end)

hook.Add("PrePlayerDraw", "luna.Observer-DrawPhysgunBeam", function(client)
    if (client:GetMoveType() == MOVETYPE_NOCLIP and !client:InVehicle()) then
		return true
	end
end)

hook.Add("Think", "luna.Observer-Think", function()
    if (!LocalPlayer():GetNetVar("observerLight")) then return end
...
hook.Add("DrawPointESP", "luna.Observer-DrawPointESP", function()

end)

hook.Add("HUDPaint", "luna.Observer-HUDPaint", function()
    local client = LocalPlayer()

	local drawESP = hook.Run("ShouldDrawAdminESP")
    
	if (drawESP == nil) then
		drawESP = luna.observer.enabled and client:GetMoveType() == MOVETYPE_NOCLIP and
	...
	if (drawESP) then
        ...
        hook.Run("DrawPlayerESP", client, scrW, scrH, luna.observer.player)

		if (luna.observer:ShouldRenderAnyTypes()) then
			for _, ent in pairs(ents.GetAll()) do
				if (!IsValid(ent)) then continue end

				local class = string.lower(ent:GetClass())
				if (luna.observer.types[class] and luna.observer.types[class][1]) then
					...
					if ((x != screenPosition.x or screenPosition.y != y) and !luna.observer.types[class][3]) then
						continue
					end

					local distance = client:GetPos():Distance(ent:GetPos())
					local factor = 1 - math.Clamp(distance / luna.observer.dimDistance, 0, 1)
					luna.observer.types[class][2](client, ent, x, y, factor, distance)
				end
			end
		end
		...
		for _, v in ipairs(points) do            
			...
			draw.SimpleText(v[2], luna.MontBase18, x, y - (size + 5), ColorAlpha(drawColor, alpha), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, nil, alpha)
		end
	end
end)
...
hook.Add("GetPlayerESPText", "luna.Observer-GetPlayerESPText", function(client, toDraw, distance, alphaFar, alphaMid, alphaClose)
...
hook.Add("PreRender", "luna.Observer-PreRender", function()
    if (LocalPlayer():GetNetVar("observerLight") and luna.observer.fullbright) then
		render.SetLightingMode(1)
	end
end)

hook.Add("PreDrawHUD", "luna.Observer-PreDrawHUD", function()
    if (LocalPlayer():GetNetVar("observerLight") and luna.observer.fullbright) then
...
hook.Add("PostDraw2DSkyBox", "StormFox2.SkyBoxRender", function()
	if (LocalPlayer():GetNetVar("observerLight") and luna.observer.fullbright) then return end
...
luna.observer.traceFilter = {nil, nil}
...
do
    ...
    local function staticESP(client, entity, x, y, factor, distance)
        ...
        if (IsValid(entity) and entity:GetNetVar("Persistent", false)) then
            draw.SimpleText(entity:GetModel(), luna.MontBase18, x, y - math.max(10, 32 * factor), espcol, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, nil, alpha)
        end
    end
    luna.observer:RegisterESPType("prop_physics", staticESP, "static", "Show Static Prop ESP")
end
...
hook.Add("DrawPlayerESP", "luna.Observer-DrawPlayerESP", function(client, scrW, scrH, drawMdl)
    local pos = client:EyePos()
    local marginX, marginY = scrW * .1, scrH * .1
    luna.observer.traceFilter[1] = client

    local names = {}
    cam.Start3D()
        local targets = hook.Run("GetAdminESPTargets") or player.GetAll()
        for _, v in ipairs(targets) do
            if (v == client or !v:Alive() or client:GetAimVector():Dot((v:GetPos() - pos):GetNormal()) < 0.65) then
                continue
            end
    ...
    for _, info in ipairs(names) do
        ...
        surface.SetFont(luna.MontBase18)
        draw.SimpleText(ply:Name(), luna.MontBase18, barX, barY - 13, teamColor, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, nil, 255)

        if (luna.observer.steam) then
            surface.SetFont(luna.MontBase18)
            ...
            for _, v in ipairs(toDraw) do
                ...
                draw.SimpleText(v.text, luna.MontBase18, barX, y, v.color or extraColor, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, nil, v.alpha)
...
hook.Add("RenderAdminESP", "luna.Observer-RenderAdminESP", function(client, target, color, clientPos, targetEyePos, distance)
    if __NOESP then return end
    render.SuppressEngineLighting(true)
    render.SetColorModulation(color.r / 255, color.g / 255, color.b / 255)

    luna.observer.traceFilter[2] = target
    if (luna.observer.blur or util.QuickTrace(clientPos, targetEyePos - clientPos, luna.observer.traceFilter).Fraction < 0.95) then
        render.SetBlend(1)
    else
